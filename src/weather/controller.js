...
class WeatherController {
  ...
  async run() {
    ...
    axios.get(`https://api.openweathermap.org/geo/1.0/direct?q=${this.city}&limit=5&appid=${WeatherConfig.ApiKey}`)
      .then(cityResponse => {
        ...
        if (cityData == null) {
          Errors.warn("Warning: Randomly selected city could not be found.");
          return;
        }
        ...
        .then(response => {
          ...
          if (this.weather == null) {
            Errors.warn(`Warning: Weather could not be found for ${this.city}`);
            return;
          }
  ...
  static render(weather) {
    if (weather === null || weather === undefined) {
      Errors.warn("Warning: Weather data could not be displayed.");
      return;
    }
